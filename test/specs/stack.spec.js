...
// These tests aren't applicable on browsers that don't have error stacks
if (host.error.stack) {
  ...
  describe("Lazy stacks", () => {
      ...
      function makeError () {
        class MyCustomError extends Error {
          constructor (message) {
            ...
            delete this.stack;
            let stack = createFakeStack(
              { fn: "foo", file: "foo.js", line: 1, col: 1 },
              { fn: "bar", file: "bar.js", line: 100, col: 100 },
            );
    ...
    it("should lazily build the new error's stack", () => {
      ...
      if (host.error.stack.includesErrorMessage) {
        expect(outerError.stack).to.match(/^Error: This is a fake error\n/);
        expect(outerError.stack).to.match(/ReferenceError: This is the original error/);
      }
    });
    ...
    it("should lazily build the new error's stack and lazily read the original error's stack", () => {
      ...
      if (host.error.stack.includesErrorMessage) {
        expect(outerError.stack).to.match(/^Error: This is a fake error\n/);
        expect(outerError.stack).to.match(/\n\nError: This is a fake error\n/);
      }
    });
